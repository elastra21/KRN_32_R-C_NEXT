...
void setup() {
  ...
  // start the UI
  ui_init();
  splash_time = millis() + SPLASH_TIMEOUT;
}
...
void loop() {
  if (splash_time > 0 && millis() > splash_time) {
    splash_time = 0;
    lv_scr_load_anim( ui_home, LV_SCR_LOAD_ANIM_MOVE_LEFT, 200, 0, false);
  }
  lv_timer_handler();
  delay(5);
}